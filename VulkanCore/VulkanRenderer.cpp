 ...
 * @brief Megvalósítja a VulkanRenderer osztályt (Javítva: Helyes 0..1 mélységtérkép az árnyékhoz).
 */
...
// --- JAVÍTOTT MÁTRIX GENERÁLÁS (0..1 Z-távolság a Vulkanhoz) ---
glm::mat4 VulkanRenderer::getLightSpaceMatrix(glm::vec3 lightPos) {
    float near_plane = 1.0f;
    float far_plane = 20.0f;

    // Manuális ortografikus mátrix építés, hogy garantáltan 0..1 legyen a mélység.
    // Ez pontosan egyezik a shader.vert-ben lévő kézi számítással!
    // X, Y: -10..10
    float left = -10.0f;
    float right = 10.0f;
    float bottom = -10.0f;
    ...
    lightProjection[2][2] = 1.0f / (near_plane - far_plane); // 1 / (near - far) -> 0..1 mapping
    lightProjection[3][2] = near_plane / (near_plane - far_plane); // near / (near - far)
    lightProjection[3][3] = 1.0f;

    // Nézeti mátrix (LookAt)
...
void VulkanRenderer::drawFrame(VulkanSwapchain* swapchain, VulkanPipeline* pipeline, glm::vec3 cameraPos, const std::vector<MeshObject*>& objects) {
    ...
    // Padló (utolsó elem) kihagyása az árnyék pass-ból
    ...
    // --- 2. PASS: FŐ RENDERELÉS (Main Pass) ---
